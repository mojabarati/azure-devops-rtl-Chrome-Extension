...
These explanations match the version 1.1.0 manifest.

## Chrome API permission: `storage`

The extension uses `chrome.storage.local` only to remember the Boolean `rtlFixEnabled` ON/OFF preference across page reloads and browser restarts. No Azure DevOps or GitHub page content, URLs, organization, project, repository information, or user identifiers are stored.
